...
namespace Microsoft.Terrapin.Worker.PrebuildLogic
{
    ...
    public static class PrebuildNpm
    {
        ...
        public static async Task<int> CleanIntegrityFieldsAsync()
        {
            ...
            string lockText = JsonSerializer.Serialize(lockData);
            var rxText = "\"integrity\":";
            var rx = new Regex(rxText, RegexOptions.Compiled);
            int integrityCount = rx.Matches(lockText).Count;

            Console.WriteLine($"-*-* Found {integrityCount} integrity matches");

            await File.WriteAllTextAsync(LockFileName, lockText);
